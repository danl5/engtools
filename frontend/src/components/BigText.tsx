...
export default function BigText({ label, value, onChange, minRows = 10, maxRows = 30, readOnly = false, onExecute, downloadName }: { label: string; value: string; onChange?: (v: string) => void; minRows?: number; maxRows?: number; readOnly?: boolean; onExecute?: () => void; downloadName?: string }) {
  ...
  return (
    <Box>
      ...
      <textarea
        ...
        rows={minRows}
        style={{ width: '100%', resize: 'vertical', minHeight: `${minRows*24}px`, maxHeight: `${maxRows*24}px`, fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace', fontSize: 14, background: 'rgba(255,255,255,0.06)', color: 'inherit', borderRadius: 12, border: '1px solid rgba(255,255,255,0.12)', padding: 16, whiteSpace: wrap ? 'pre-wrap' : 'pre', outline: 'none' }}
        readOnly={readOnly}
      />
      <Dialog open={open} onClose={()=>setOpen(false)} fullWidth maxWidth="lg">
        <DialogContent>
          <textarea
            ...
            onKeyDown={e=>{ if ((e.ctrlKey || e.metaKey) && e.key === 'Enter' && onExecute) { e.preventDefault(); onExecute() } }}
            rows={24}
            style={{ width: '100%', minHeight: '70vh', fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace', fontSize: 14, background: 'rgba(255,255,255,0.06)', color: 'inherit', borderRadius: 12, border: '1px solid rgba(255,255,255,0.12)', padding: 16, whiteSpace: wrap ? 'pre-wrap' : 'pre', outline: 'none' }}
            readOnly={readOnly}
          />
        </DialogContent>
      </Dialog>
