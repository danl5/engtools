...
if [[ ! -f deploy/.env ]]; then mkdir -p deploy; JWT_SECRET=$(openssl rand -hex 32 || dd if=/dev/urandom bs=32 count=1 2>/dev/null | xxd -p); printf "BACKEND_PORT=%s\nFRONTEND_PORT=%s\nJWT_SECRET=%s\nALLOW_ORIGIN=%s\nBACKEND_ADDR=:%s\nIPINFO_TOKEN=%s\n" "$BACKEND_PORT" "$FRONTEND_PORT" "$JWT_SECRET" "$ALLOW_ORIGIN" "$BACKEND_PORT" "$IPINFO_TOKEN" > deploy/.env; else
